...
public class Day23 {
    ...
    private int findMinCost(boolean part2) {
        ...
        queue.add(new SearchNode(new State(getPuzzleInput(_filename), part2).encode(), 0));
        while (!queue.isEmpty()) {
    ...
    private static class State {
        ...
        private State(String input, boolean part2) {
            ...
            for (var i = 0; i < input.length(); i++) {
                ...
                switch (c) {
                    case '.' -> state[j++] = null;
                    case 'A' -> state[j++] = Amphipod.AMBER;
                    case 'B' -> state[j++] = Amphipod.BRONZE;
                    case 'C' -> state[j++] = Amphipod.COPPER;
                    case 'D' -> state[j++] = Amphipod.DESERT;
                }
            }
            if (part2) {
                System.arraycopy(state, 15, state, 23, 4);
                var extra = new Amphipod[] { Amphipod.DESERT, Amphipod.COPPER, Amphipod.BRONZE, Amphipod.AMBER, Amphipod.DESERT, Amphipod.BRONZE, Amphipod.AMBER, Amphipod.COPPER };
